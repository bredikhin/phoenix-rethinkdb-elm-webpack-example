...
just can't quit), and finally you realize that there's no such thing as the
perfect stack. And the same applies to frameworks.
...
pivot, and you're gonna have to find a way to follow, but that's a
different story.

So, a particular kind of applications we wanna talk about today is those
...
So, of course, there are already some great tools for it out there, and
the first thing that comes in mind is, obviously, WebSocket Protocol, which
we'll be using as well. If you look at some of its most popular
implementations that were out there during the last years, a lot of
them have their server side written in Node.js (which somewhat makes sense
since the client side is almost always in JavaScript). This includes
Socket.io / Engine.io, Sockjs and based on it Meteor.js. The latter, in
fact, is one of the most impressive platforms for the real-time applications,
and I'd even go further and say it's one of the best for prototyping.
...
What we're gonna describe here is a real-time stack based on Phoenix
Framework (written in Elixir language) and RethinkDB (which is a scalable
JSON database with built-in real-time features). In order to move even
further from JavaScript, we'll also try something different for our
client-side: Elm is a functional language that compiles to JavaScript while
providing some amazing performance and enhanced development efficiency.

So, this guide is essentially about making it all working together in an
this extremely innovative example of an application which is a todo list.
...
me and you appreciated the natural beauty of a good script written in Ruby,
you're gonna love Elixir, since on top of that awesome syntax it doesn't
...
Homebrew: you can just type `brew update && brew install elixir` and relax.
Otherwise, just pick a guide for your OS and you should be fine.

The version we'll be using here is v1.4, which is the latest at the time of
writing.

Now, once Elixir is installed, let's proceed with Phoenix.

Phoenix is a productive, reliable and fast Web application framework for
Elixir language. Let's mention right away that it's very different from
Ruby on Rails in both its core and details; however, they do share some
values (some API patterns and principles are borrowed from Rails, for
example), and one can say that Phoenix is for the Elixir community the
same thing Rails is for the Ruby one: the framework.
...
we're gonna need to work later with Elm anyway, so go ahead and install it)
and a section on PostgreSQL (which you can safely skip, since we're gonna
be using RethinkDB instead). Also, you can easily reply 'no' to the question
about installing the dependencies, since we're not gonna be using the default
...
If you're on a Mac, the easiest way is probably to use Homebrew:
...
forget to `git init` at this point in case you wanna version control your
steps.

## Add RethinkDB Ecto adapter and create you database
...
let's say, PostgreSQL ones. However, having schema definition and migrations
...
So, let's set it up. Add `rethinkdb_ecto` to your dependencies:
...
You can also remove `postgrex` since we're not gonna use it.
...
The reason we need them both is that we're gonna use this configuration with
...
impression only, we're just gonna use one table, the one that'll store
our todos.
...
Phoenix comes with Brunch configuration for managing application assets,
but we're gonna use Webpack instead, so the first thing we should do is
removing the Brunch configuration file: `rm assets/brunch-config.js`.
Let's remove Brunch dependencies from `package.json` as well:
...
Now, since Phoenix uses ES2015 syntax for its JavaScript files, we're gonna
...
requests (where to look for imports), which is as simple as adding the
following line to the `assets/webpack.config.js`:
...
Update `assets/webpack.config.js` to include the CSS source, the loader and the
plugin extracting CSS in a separate file:
...
gonna do with his beautiful code further) as the template for our frontend:
...
we could see the app working, but we're gonna remove it once we connect our
Elm code to the database via a Phoenix channel.

Okay, after that piece of JavaScript is copied, you can safely delete
`elm/index.html`, since we're got all we needed out of it.
...
Let's just go ahead and created a channel, so we could learn it all in
practice:
...
already create all the necessary functions with example signatures / bodies,
so all we need to do is to open `lib/rephink/web/channels/todo_channel.ex` and
edit it according to our goals:
...
authorization, but not gonna elaborate on it in this post considering for
sake of simplicity that we only have one common task list and everybody can
read and modify it. As for the "ping" message, it's a temporary one, let's
...
came with the original app, since we're not gonna use it. In `elm/Todo.elm`,
...
Next stop: we need to add some new messages to or `Msg` type:
...
(here we've also added a logger, so we could do some analysis).
...
We'll use `ReceiveEntries` message to do it:
```
...
        ReceiveEntries raw ->
            let
...
Done? Let's try it out. At this point we're just gonna add some todos manually
...
that it's not gonna be written in the database. But we'll fix it in a bit.
...
On the client side, it's a little more work. First, a little side note: in
...
And this gets extremely handy when we're gonna try inserting our new record in
the database:
```
...
update msg model =
...
Then, on the client side we're gonna be changing the `EditingEntry` message
and adding a new one called `SyncEntry`:
```
...
update msg model =
        ...
...
What's happening here is that we're sending our updated entry to the server
...
since it issues the number of messages essentially equal to the number of our
...
was kinda cool to the application getting real-time updates for free, and
it even was (almost) scalable. So, RethinkDB changefeeds let you essentially
get the same functionality, once again, for free. Well, almost.
...
So, how do we arrange it? Let's say, we wanna sync our todo list across
multiple devices and get real-time pushes whenever we edit it on one of them?
Let's see.

First, we're gonna be using a dedicated library to deal with the changefeeds:
https://github.com/hamiltop/rethinkdb_changefeed. Configuration is pretty
straightforward: we just need to add `{:rethinkdb_changefeed, "~> 0.0.1"}` to
the list of the dependencies and start `:rethinkdb_changefeed`
with all the other `extra_applications` in the `application` in our `mix.exs`.
...
defmodule Rephink.Changefeed do
  ...
  def handle_update(data, {db, todos}) do
    ...
    Rephink.Web.Endpoint.broadcast!(@topic, @table_name, %{data: Map.values(todos)})
...
What we got ourselves here is essentially a `GenServer` that holds the current
list of todos as it's state and updates it whenever the database subscription
...
touch in order to improve the structure of our code. We're gonna move the
...
you're gonna feel like you need to scale horizontally, you can easily leverage
the power of distributed Elixir and RethinkDB clustering.
